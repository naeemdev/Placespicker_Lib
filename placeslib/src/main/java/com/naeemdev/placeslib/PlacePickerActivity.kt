...
class PlacePickerActivity : AppCompatActivity() , OnMapReadyCallback {
    ...
    fun init(){
        ...
        placeSelectedFab.setOnClickListener {
            if (onlyCoordinates) {
                ...
                if (addresses != null) {
                    val addressData = AddressDataClass(latitude, longitude, addresses)
    ...
    private fun sendOnlyCoordinates() {
        val addressData = AddressDataClass(latitude, longitude, null)
        val returnIntent = Intent()
        returnIntent.putExtra(Constants.ADDRESS_INTENT, addressData)
        setResult(RESULT_OK, returnIntent)
        finish()
    ...
    private fun setIntentCustomization() {
        ...
        if (fabColorRes != -1) {
            placeSelectedFab.backgroundTintList = ColorStateList.valueOf(
                ContextCompat.getColor(
                    this,
                    fabColorRes
                )
            )
            myLocationFab.backgroundTintList = ColorStateList.valueOf(
                ContextCompat.getColor(
                    this,
                    fabColorRes
                )
    ...
    private fun setPlaceDetails(
        ...
        placeCoordinatesTextView.text = Location.convert(latitude, Location.FORMAT_DEGREES) + ", " + Location.convert(
            longitude,
